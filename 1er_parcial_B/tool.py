...
class SprayTool:
    ...
    def make_spray_points(self, x: int, y: int, count: int = 80, radius: int = 24) -> List[Tuple[int, int]]:
        ...
        for _ in range(count):
            ...
            sx = int(x + r * random.cos(angle) if hasattr(random, 'cos') else x + r * __import__('math').cos(angle))
            sy = int(y + r * random.sin(angle) if hasattr(random, 'sin') else y + r * __import__('math').sin(angle))
            points.append((sx, sy))
        return points
    def draw_traces(self, traces: List[Dict[str, Any]]):
        for trace in traces:
